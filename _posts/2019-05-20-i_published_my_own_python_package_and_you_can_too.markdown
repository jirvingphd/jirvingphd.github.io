...
1. **Becuase you're *sick* of having to copy and paste your favorite functions into every new notebook.**
 - I work on several computers and also in the cloud. It was always a pain to have to log into Dropbox on any new computer and remember where I saved that notebook that had all of those neat little functions I wrote... 

2. **Because you collaborate with others and want to ensure you all have the same toolkit at your disposal.**
 - My collaborator and fellow student, [Michael Moravetz](https://github.com/MichaelMoravetz), and I have been working closely together, and we wanted an easy way to share the cool/helpful tools each of us that we either wrote ourselves or were provided in our Bootcamp lessons. 

3. **Because you're instructor tells you that your notebook has WAY too many functions upfront, and it is distracting...**
 - ***You:*** "But aren't I *supposed* to write a lot of functions to become a better programmer?"
 - ***Me:*** "Yes...you're right... BUT when someone has to scroll through a lot of functions-as-appetizers before getting to your main-course-notebook, they may not have much attentional-appetite left."
...
5. **Finally, because you're lazy** and just want all of your tools imported for you and ready to use wherever you go with as little effort as possible.
...
* Please note: there are simpler ways to go about creating your first Python module, but I am going to show you the exact tools and instructions I used to allow me to have a completely automated workflow and package deployment.* 
...
- [Microsoft's Visual Studio Code](https://code.visualstudio.com/docs/introvideos/basics)
 - Its optionally installed with Anaconda and has quickly become my favorite editor. *(Sorry, SublimeText3... It's not you! It's me.<br> I still love you..I'm just not as ***in love*** with you as I used to be. But we can still be friends, right?!)*
- [GitHub Desktop](https://desktop.github.com/)
 - It is SO convenient and simplifies the workflow and collaboration immensely.
...
- [ ] **Open Cookiecutter's [official tutorial from their documentation.](https://cookiecutter-pypackage.readthedocs.io/en/latest/tutorial.html)**
 - It is very good overall, but I suggest doing a few steps a little differently. My suggested steps will start with "REC'D" to indicate deviations from the official steps.
...
2. [ ] **Register your GitHub account with www.travis-ci.org. [OPTIONAL, BUT HIGHLY RECOMMENDED]**<br> for automated build testing and auto-deployment to PyPi.
 - NOTE: Make sure to go to **www.travis-ci.ORG** (**NOT** travis-ci.**COM**, which is for commercial, non-open source packages) <br><br>
 - **Suggestion: This is totally worth it.** It adds a little complexity to the cookie-cutting setup process, but it:
...
- [ ] **Create a new virtual environment**, preferably by cloning your current one.
 ...
 - Click on your current environment of choice, then click the Clone button and give it a new name. 
 - [ ] Add this to your Jupyter notebook kernels using <br>```python -m ipykernel install --user --name myenv```
- [ ] **Backup and export your current environment** to a .yml file, which you can use to re-install your env if need be. 
 ...
 - This command will save the .yml into your current directory. The .yml can be used to install your environment in the future using:<br>```conda env create -f my_environment.yml```
- **DO NOT SKIP THIS STEP.** I have warned you, and I am not responsible for any broken environments.<br>While nothing *should* break; it's always a GOOD idea to create a new environment for creating and installing test packages. I should say its a DUMB idea not to. <br><br><br>
...
***NOTE: My recommendation deviates from the tutorial. These steps will replace "Step 3: Create a GitHub Repo".***
- [ ] Log into your GitHub profile on github.com and Create a New Repository <br>by clicking the + sign next to your account picture on the top-right of the page. 
 ...
 - Leave the rest of the options blank/none. ```Initialise with Add a .git ignore```, ```Add a license```
...
- **Cookiecutter will ask you several questions during the cookie-cutting process, [check this resource to see the descriptions for each prompt.](https://cookiecutter-pypackage.readthedocs.io/en/latest/prompts.html)**
...
- **"Select open source license"**
 - Choosing a license a critical decision that determines what other people are allowed to do with your code with or without your permission. 
...
- If you followed my REC'D STEP #2, your main repo folder should now contain:
 - a README.md file
 - a ".git" folder
 ...
 - Inside of the project_slug folder, there should only be 2 files and 0 folders:
 - __init__.py
 - project_slug.py
...
- [ ] **Once Travis CLI is installed, you may continue to follow the [official tutorial instructions for step #5](https://cookiecutter-pypackage.readthedocs.io/en/latest/tutorial.html#step-5-set-up-travis-ci)**
 ...
 - **TROUBLESHOOTING NOTE: If Travis hangs and does not ask for your password after entering your username:**
 ...
 - I use Git Bash for my main terminal on Windows and for some reason **Travis would hang after I entered my username and would never ask me for the password.**
 - I got around the issue by **using the standard windows cmd prompt for this step instead of using GitBash.** (This is a one-time step that will encrypt your password and store it in a config file, so you never have to enter it again.)
...
- [ ] To *register* your new package with PyPi for the very first version, you must **manually create and upload the very first version** of your package. [Official Python instructions for "generating distribution archives," summarized below](https://packaging.python.org/tutorials/packaging-projects/#generating-distribution-archives)
 ...
 - For me, using ```python3``` for the above commands did not work. I simply had to change ```python3``` to just ```python```<br>
 - Example:<br>
 ```python -m pip install --user --upgrade setuptools wheel```
 - If this doesn't fix it for you, you may need to update your system's Path variable (basically a list that tells your computer all of the locations on your PC where you may have scripts/functions saved to run from your terminal).
...
- **The easiest way to manage all of your package's setup files and modules is to the File > Open Folder option and select your repo's main folder.**
...
- At the bottom of the file, there is a ```deploy:``` section. 
 - I personally had difficulty using ```--tags``` in order to trigger the deployment of bs_ds.
